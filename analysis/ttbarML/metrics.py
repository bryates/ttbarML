...
def net_eval(out, test_feat, test_input, threshold=0.5, n_points=200):
    
    #sm_hist, bsm_hist = make_hist(out, sm, bsm, n_points)
    
    roc, auc = make_roc(out, test_feat, n_points)
...
def make_roc(out, test_feat, n_points):
    
    roc = torch.cat((torch.cumsum(out, dim=0),#.reshape(n_points,1),
                     torch.cumsum(test_feat, dim=0)), axis=1)#.reshape(n_points,1)), axis=1)
    roc = 1 - roc/roc[-1]
